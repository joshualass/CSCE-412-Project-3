 ...
 * @param argv The command-line arguments (total_clock_cycles, num_requests, n, firewall_lower_range, firewall_upper_range).
...
int main(int argc, char **argv) {
    ...
    cout << "Task Time Range: 10 to 29 clock cycles" << endl;
    ...
    for (const auto& lb : s.loadBalancers) {
        ...
        // Loop from 0 to the total number of servers owned by this LB
        for (size_t k = 0; k <= lb.webservers.size(); k++) {
            // Check if we logged any cycles at this 'k' amount; if not, default to 0
